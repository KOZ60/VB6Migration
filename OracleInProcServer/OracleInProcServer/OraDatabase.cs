...
namespace OracleInProcServer
{
    public class OraDatabase : OraObject
    {
        ...
        private DbCommand CreateDbCommand(string sqlStmt, bool autoBind) {
            ...
            // ODP.NET 固有
            cmd.SetProperty("BindByName", true);
            cmd.SetProperty("InitialLOBFetchSize", -1);
            cmd.SetProperty("InitialLONGFetchSize", -1);
            cmd.SetProperty("FetchSize", 1024 * 512);
            ...
            if (autoBind) {
                ...
                if (nArrayBindCount != 0) {
                    cmd.SetProperty("ArrayBindCount", nArrayBindCount);
                }
            }
            return cmd;
        }
        ...
        public int ExecuteSQL(string sqlStmt) {
            using (DbCommand cmd = CreateDbCommand(sqlStmt, true)) {
                return cmd.ExecuteNonQuery();
            }
        }

        public OraDynaset CreateDynaset(string sqlStmt, dynOption options) {
            using (DbCommand cmd = CreateDbCommand(sqlStmt, !options.HasFlag(dynOption.ORADYN_NO_AUTOBIND))) {
